...
def ssa_to_linear(ssa_path):
    ...
    linear ids. For example::
        ...
        [(0, 3), (1, 2), (0, 1)]
    """
    ids = np.arange(1 + max(list(map(max, ssa_path))), dtype=np.int32)
    path = []
...
def calc_k12_flops(inputs, output, remaining, i, j, size_dict):
    ...
    either = k1 | k2
    shared = k1 & k2
    keep = frozenset.union(output, *list(map(inputs.__getitem__, remaining - {i, j})))
...
def _compute_oversize_flops(inputs, remaining, output, size_dict):
    ...
    """
    idx_contraction = frozenset.union(*list(map(inputs.__getitem__, remaining)))
...
def ssa_greedy_optimize(inputs, output, sizes, choose_fn=None, cost_fn='memory-removed'):
    ...
    dim_ref_counts = {
        count: set(dim for dim, keys in list(dim_to_keys.items()) if len(keys) >= count) - output
        for count in [2, 3]}
    ...
    queue = []
    for dim, keys in list(dim_to_keys.items()):
    ...
    queue = [(helpers.compute_size_by_dict(key & output, sizes), ssa_id, key)
             for key, ssa_id in list(remaining.items())]
    heapq.heapify(queue)
...
class DynamicProgramming(PathOptimizer):
    ...
    def __call__(self, inputs, output, size_dict, memory_limit=None):
        ...
        size_dict = {symbol2int[c]: v for c, v in list(size_dict.items()) if c in symbol2int}
        ...
        for g in subgraphs:
            ...
            cost_increment = max(min(list(map(size_dict.__getitem__, subgraph_inds))), 2)
            ...
            while len(x[-1]) == 0:
                for n in range(2, len(x[1]) + 1):
                    ...
                    for m in range(1, n // 2 + 1):
                        for s1, (i1, cost1, cntrct1) in list(x[m].items()):
                            for s2, (i2, cost2, cntrct2) in list(x[n - m].items()):
        ...
        subgraph_contractions = [
            ...
            for j in sorted(list(range(len(subgraph_contractions_size))), key=subgraph_contractions_size.__getitem__)
        ]
