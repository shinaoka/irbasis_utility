...
Nl = phb.Nl
#sp = phb.sampling_points_matsubara(whichl=phb.Nl-1)
#print(len(sp), len(freqs_PH))
#print(sp)
#print(freqs_PH)
#assert numpy.allclose(numpy.array(sp), freqs_PH[:,:2])
...
def kruskal_complex_Ds(tensors_A, y, Ds):
    ...
    for i, D in enumerate(Ds):
        ...
        model = OvercompleteGFModel(Nw, Nr, 2, nflavors**4, linear_dim, tensors_A, y.reshape((-1,nflavors**4)),
                                    alpha_init, D)
        info = optimize_als(model, 20, tol_rmse = 1e-6,
